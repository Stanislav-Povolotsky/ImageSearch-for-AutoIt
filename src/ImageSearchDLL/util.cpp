...
char* g_empty_answer = "";
char* g_answer_cache = g_empty_answer;
int g_answer_cache_cur_len = 0;
int g_answer_cache_reserved_len = 0;
...
void Answer_Clear(int final = 0)
{
    g_answer_cache_cur_len = 0;
    if (final || !g_answer_cache || g_answer_cache == g_empty_answer || (g_answer_cache_reserved_len == 0))
    {
        g_answer_cache_reserved_len = 0;
        if (g_answer_cache != g_answer_cache) {
            free(g_answer_cache);
        }
        g_answer_cache = g_empty_answer;
    }
    else 
    {
        g_answer_cache[0] = 0;
...
char* Answer_Append(const char* s)
{
    int slen = strlen(s);
    while (slen)
    {
        int newlen = g_answer_cache_cur_len + slen + 1;
        if (newlen > g_answer_cache_reserved_len)
        {
            g_answer_cache_reserved_len = newlen + (newlen >> 1) + 100;
            g_answer_cache = (char*)((g_answer_cache == g_empty_answer) ?
                malloc(g_answer_cache_reserved_len) : realloc(g_answer_cache, g_answer_cache_reserved_len));
            if (!g_answer_cache) {
                Answer_Clear();
                break;
            }
...
// in which case the contents of the output parameters is indeterminate.
{
    ...
    {
        PALETTEENTRY local_palette[256];
        ...
        // 32-bit RGB color.
        LPDWORD palette = (LPDWORD)local_palette;
...
char* IntAddResult(int i, int aLeft, int aTop, RECT* pRect, int screen_width, int image_width, int image_height)
{
    char answer[50];
...
char* WINAPI ImageSearch(int aLeft, int aTop, int aRight, int aBottom, char *aImageFile)
{
    ...
    while (*cp == '*')
    {
        ...
        case 'M': b_multi_results = 1; break;
        default:
    ...
    if (aVariation < 1) // Caller wants an exact match.
    {
        ...
        for (i = 0; i < screen_pixel_count; ++i)
        {
                ...
                && image_width <= screen_width - i % screen_width)  // Image is narrow enough not to exceed the right-side boundary of the search region.
            {
                ...
                for (found = false, x = 0, y = 0, j = 0, k = i; j < image_pixel_count; ++j)
                {
                ...
                {
                    ++n_total_found;
                    IntAddResult(i, aLeft, aTop, &rect, screen_width, image_width, image_height);
                    if (!b_multi_results) break;
                    found = false;
                }
            }
        }
...
good_end:
    ...
    if (n_total_found && g_answer_cache && *g_answer_cache) {
        return g_answer_cache;
    }
    return "0";
}
